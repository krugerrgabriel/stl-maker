...
#
# Uso:
#   sudo ./install.sh      instala (requer sudo)
#   ./install.sh --check   apenas verifica a instalação (não precisa de sudo)
#
...
instala_python() {
  ...
  if [ ! -x "$VENV/bin/python" ]; then
    ...
    como_usuario python3 -m venv "$VENV" || { vermelho "Falha ao criar venv."; return 1; }
  fi
  info "Instalando pacotes Python (build123d é grande, pode demorar alguns minutos)..."
  como_usuario "$VENV/bin/pip" install --quiet --upgrade pip \
    && como_usuario "$VENV/bin/pip" install --quiet "${PACOTES_PY[@]}" \
...
if [ "$(id -u)" != 0 ]; then
  vermelho "A instalação requer sudo. Rode: sudo ./install.sh"
  exit 1
fi
